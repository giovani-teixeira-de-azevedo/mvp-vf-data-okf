...
title: Parameters
description: Configuration parameters, value ranges, datatypes, and default settings
  for the CQI-Based UE Energy Efficiency Enhancement feature.
tags:
- cqi
- parameters
- energy-efficiency
...
This section details the configuration parameters, value ranges, data types, and default values for the CQI-Based UE Energy Efficiency Enhancement feature.

## Parameter Guidance

Thresholds are expressed on the standard 0–15 CQI scale (4-bit wideband CQI, TS 38.214). The defaults suit a typical mid-band macro cell; indoor cells with compressed CQI distributions may lower `highCqiThr` by one step. Keep at least 3 CQI steps between `lowCqiThr` and `highCqiThr`.
