...
- **Modulation-Aware Power Control:** When both features are active, modulation-specific power offsets are applied first, followed by the energy headroom reduction on top, bounded by a combined floor.
- **EMF Power Lock Mid-Band:** Compatible with EMF Power Lock Mid-Band. The EMF lock defines the upper power bound, while Energy-Optimized Power Allocation only reduces power below this limit.
- **NR Massive MIMO Eco Mode:** On cells where Eco Mode caps total power, headroom evaluation uses the capped power as reference.
