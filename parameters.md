...
title: PARAMETERS
description: Configurable sector carrier parameters for Closed-Loop Power Control
  High-Band.
tags:
- parameters
...
This section defines the configuration parameters for the Closed-Loop Power Control High-Band feature. The parameters are configured per NR sector carrier.

The target SINR (`pcTargetSinr`) serves as the primary tuning knob: raising it improves per-UE MCS at the cost of higher interference, with default settings selected for dense urban FR2 grids. Adjustments to the hysteresis window and step configuration should only be made after observing loop stability counters.
